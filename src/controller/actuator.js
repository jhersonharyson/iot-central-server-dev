...
export async function getActuator(req, res, next) {
  ...
  res.send(await Actuator.find(type ? { type } : {}).sort([["createAt"]]));
}

export async function updateActuator(req, res) {
  try {
    const inf = await Actuator.findOneAndUpdate(
      { _id: req.params.id },
      { useAndModify: true }
    );
    ...
    if (inf.type == "INFLATOR") {
      req.io.emit("updateActuatorInflator", inf);
    } else {
      req.io.emit("updateActuatorGenerator", inf);
    }
    res.send(inf);
  } catch (err) {
...
export async function deleteActuator(req, res) {
  ...
  await Actuator.deleteOne({ _id: id })
    .then(arr => {
      if (arr.n) {
        res.send({ status: "ok" });
      } else {
