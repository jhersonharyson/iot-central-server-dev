...
export default setInterval(async () => {
	...
	const device = await Device.find({status: 1}).select('sensorData').populate({
		path: 'sensorData',
		select: 'createAt',
		options: {
			limit: 1,
	...
	device.map( async arr => {
		if(arr.sensorData[0]){
			...
			let a = moment(data).add(tolerance + parseInt(varDevice.sendTime), 'seconds');
			let der = a.diff(moment(),'seconds');
			if(der < 0) {
				disableDevice(arr._id)
			} 
		} else {
			disableDevice(arr._id)
		}
